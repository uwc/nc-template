...
function nc_template_customize() {
	wp_die( sprintf( esc_attr( __( 'NC Template requires at least WordPress version 4.4. You are running version %s. Please upgrade and try again.', 'nc-template' ) ), $GLOBALS['wp_version'] ), '', array(
		'back_link' => true,
	) );
}
...
function nc_template_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( sprintf( esc_attr( __( 'NC Template requires at least WordPress version 4.4. You are running version %s. Please upgrade and try again.', 'nc-template' ) ), $GLOBALS['wp_version'] ) );
	}
}
